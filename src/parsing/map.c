...
/*   Updated: 2024/06/14 11:32:38 by mcatalan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	store_map(t_cube *cube, int pos_map)
{
	...
	while ((++i - pos_map) < cube->map_h)
	{
		...
		if (!cube->map[i - pos_map])
			generic_exit("Memory allocation failed."); //erro malloc
		while (++j < cube->map_w && cube->file[i][j] != '\0')
		{
			if (cube->file[i][j] && is_empty_line(cube->file[i]) == false)
	...
	printf("Mappppppppppppppppppppppppppppp:\n");
